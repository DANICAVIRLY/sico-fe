...
const API_URL = "http://172.18.160.168:8000";
...
export default function PengajuanSaya() {
  ...
  const getPengajuan = async () => {
    try {
      ...
      const response = await axios.get(
        "http://172.18.160.168:8000/api/pengajuan-clearing",
        getConfig()
      );
  ...
  const handleAjukanUlang = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://172.18.160.168:8000/api/pengajuan-clearing/${pengajuanRevisi.id}/ajukan-ulang`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handlePreview = async (pengajuanId, jenis) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://172.18.160.168:8000/api/pengajuan-clearing/${pengajuanId}/dokumen/${jenis}`,
        {
